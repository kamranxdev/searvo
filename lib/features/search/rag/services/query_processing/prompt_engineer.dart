  ...
  String createSystemPrompt({SearchMode searchMode = SearchMode.search}) {
    switch (searchMode) {
      case SearchMode.research:
        return '''You are an advanced academic research assistant functioning as a subject matter expert. Your goal is to provide deep, comprehensive, and rigorously cited analysis.

RESPONSE STRUCTURE:
1. Executive Summary: Brief high-level overview (2-3 sentences).
2. Key Findings: Structured analysis with specific details.
3. In-Depth Analysis: Comprehensive synthesis of multiple sources, exploring nuances and conflicting viewpoints.
4. Methodology/Context: Mention the nature of sources if relevant (e.g., "Based on recent optimization papers...").
5. Conclusion: Synthesize the findings into a forward-looking perspective.

WRITING STYLE:
- Academic and professional tone.
- Use precise terminology.
- Prioritize depth and nuance over brevity.
- Explicitly highlight consensus vs. debate in the sources.

CITATION RULES:
- strict inline citations [1], [1, 2].
- Group citations for supported claims: "Multiple studies suggest... [1, 3, 5]".
- Cite immediately after the claim or sentence.
''';

      case SearchMode.study:
        return '''You are an expert tutor and pedagogical guide designed to help users learn and understand complex topics.

RESPONSE PHILOSOPHY:
- Focus on clarity, education, and understanding.
- Use analogies and examples to explain difficult concepts.
- Anticipate confusion and clarify jargon.
- Adopt a Socratic but helpful tone.

STRUCTURE:
1. Direct Answer: Simple, clear explanation of the core concept.
2. "Explain Like I'm 5" (ELI5): A simplified analogy or breakdown if the topic is complex.
3. Deep Dive: detailed explanation with technical accuracy.
4. Key Takeaways: Bullet points of the most important concepts.

CITATION RULES:
- Use citations [1] or [1, 2] to verify facts, but prioritize flow and readability.
''';
...
CITATION RULES:
- Add citations [1] or [1, 2] at the end of sentences with factual claims
- Cite after the period: "This is a fact.[1]" 
- Use multiple citations for corroborated facts: "This is reported by multiple sources.[1, 3, 5]"
- One citation group per sentence maximum unless multiple distinct facts
...
- Generic conclusions''';
  ...
  String createUserPrompt(
    ...
    if (contextChunks.isNotEmpty) {
      ...
      print(
        '📊 Prompt: ${contextChunks.length} chunks from ${uniqueSources.length} unique sources',
      );
      ...
      for (int i = 0; i < contextChunks.length; i++) {
        ...
        final sourceNum = sourceInfo != null
            ? sourceNumbers[sourceInfo.url]
            : i + 1;
    ...
    if (hasUserProvidedUrls) {
      buffer.writeln(
        '- IMPORTANT: The user has provided specific URL(s) in their query. Prioritize content from those URLs (source [1]) in your answer.',
      );
      buffer.writeln(
        '- Focus primarily on information from the user-specified URL(s)',
      );
      buffer.writeln(
        '- Use other sources only to supplement or provide additional context',
      );
    }

    buffer.writeln('- Start immediately with the core answer');
    buffer.writeln('- Cite sources [1] or [1, 2] at sentence ends');
  ...
  String createUserPromptWithHistory(
    ...
    if (contextChunks.isNotEmpty) {
      ...
      print(
        '📊 History Prompt: ${contextChunks.length} chunks from ${uniqueSources.length} unique sources',
      );
    ...
    if (conversationContext != null && conversationContext.isNotEmpty) {
      buffer.writeln(
        '- This question may relate to the conversation history above',
      );
      buffer.writeln(
        '- If it references previous topics (e.g., "who leads it?"), focus on that context',
      );
      buffer.writeln(
        '- If it\'s a new independent question, provide a complete standalone answer',
      );
      buffer.writeln(
        '- NEVER explain grammar, word meanings, or definitions unless explicitly asked',
      );
      buffer.writeln(
        '- Provide specific, factual information from the sources',
      );
    }
  ...
  String createAdaptivePrompt(
    ...
    if (comparativeKeywords.any((kw) => queryLower.contains(kw))) {
      return _createComparativePrompt(
        query,
        contextChunks,
        attachmentContext,
        hasUserProvidedUrls,
  ...
  String _createComparativePrompt(
    ...
    print(
      '📊 Comparative Prompt: ${contextChunks.length} chunks from ${uniqueSources.length} unique sources',
    );
  ...
  String _createTechnicalPrompt(
    ...
    print(
      '📊 Technical Prompt: ${contextChunks.length} chunks from ${uniqueSources.length} unique sources',
    );
...
4. Be honest about limitations while remaining helpful''';
  ...
  List<String> generateFollowUpQuestionsWithHistory(
    ...
    if (previousMessages.isNotEmpty && historicalTopics.isNotEmpty) {
      ...
      if (!queryLower.contains(recentTopic.toLowerCase())) {
        questions.add(
          'How does this connect to the $recentTopic we discussed earlier?',
        );
      }
    }
    ...
    if (hasAttachments && questions.length < 4) {
      questions.add(
        'Can you compare this with specific sections from the uploaded files?',
      );
    }

    // Ensure diversity and quality
    final uniqueQuestions = _ensureQuestionQuality(
      questions,
      currentEntities,
      currentTopics,
    );
    print(
      '   ✅ Generated ${uniqueQuestions.length} unique questions: $uniqueQuestions',
    );
  ...
  List<String> generateFollowUpQuestions(
    ...
    if (hasAttachments && questions.length < 4) {
      questions.add(
        'Can you analyze specific data from the uploaded documents?',
      );
    }
    ...
    print(
      '   ✅ Generated ${uniqueQuestions.length} questions: $uniqueQuestions',
    );
  ...
  List<String> _extractEntitiesFromQuery(String query) {
    ...
    final entityPatterns = [
      ...
      RegExp(
        r'\b(china|india|america|europe|africa|asia|russia|japan|korea|australia|'
        r'mexico|brazil|canada|france|germany|italy|spain|egypt|greece)\b',
        caseSensitive: false,
      ),

      // Organizations/Institutions
      RegExp(
        r'\b(nasa|who|un|eu|fbi|cia|google|microsoft|apple|amazon|facebook|'
        r'meta|twitter|tesla|spacex|university|institute|academy)\b',
        caseSensitive: false,
      ),

      // Historical figures/Scientists (context-based)
      RegExp(
        r'\b(darwin|einstein|newton|tesla|curie|hawking|sagan|tyson|dawkins|'
        r'galileo|copernicus|kepler)\b',
        caseSensitive: false,
      ),

      // Species/Taxonomy
      RegExp(
        r'\bhomo\s+(sapiens|erectus|habilis|neanderthalensis)\b',
        caseSensitive: false,
      ),

      // Specific archaeological sites/specimens
      RegExp(
        r'\b(yunxian|dali|peking man|lucy|turkana|olduvai)\b',
        caseSensitive: false,
      ),
    ];
  ...
  List<String> _extractDomainTerms(String text) {
    ...
    // Science & Research
    final scienceTerms = RegExp(
      r'\b(evolution|fossil|skull|dna|gene|species|homo|sapiens|erectus|'
    ...
    final techTerms = RegExp(
      r'\b(algorithm|ai|machine learning|neural network|kubernetes|docker|'
    ...
    // Medicine & Health
    final medTerms = RegExp(
      r'\b(disease|treatment|therapy|diagnosis|symptom|vaccine|virus|bacteria|'
    ...
    // Politics & Government
    final politicsTerms = RegExp(
      r'\b(election|vote|parliament|congress|senate|legislation|policy|'
    ...
    // Economics & Business
    final economicsTerms = RegExp(
      r'\b(economy|market|stock|investment|trade|gdp|inflation|recession|'
    ...
    // Environment & Climate
    final envTerms = RegExp(
      r'\b(climate change|global warming|carbon|emission|renewable energy|'
    ...
    // Legal & Justice
    final legalTerms = RegExp(
      r'\b(court|judge|trial|verdict|sentence|conviction|lawsuit|plaintiff|'
      r'defendant|attorney|lawyer|justice|law|legal|crime|prosecution|defense)\b',
    );
    domains.addAll(_extractMatches(text, legalTerms));

    // Space & Astronomy
    final spaceTerms = RegExp(
      r'\b(space|planet|star|galaxy|universe|cosmos|telescope|nasa|'
  ...
  List<String> _extractCompoundConcepts(List<String> words) {
    ...
    for (int i = 0; i < words.length - 1; i++) {
      ...
      if (i < words.length - 2) {
        final word3 = words[i + 2].toLowerCase().replaceAll(
          RegExp(r'[^\w]'),
          '',
        );
    ...
    switch (intent) {
      case 'discovery':
        if (keyFindings['subject'] != null) {
          questions.add(
            'What makes this ${keyFindings['subject']} discovery scientifically significant?',
          );

          if (topics.isNotEmpty) {
            questions.add(
              'How does this ${keyFindings['subject']} change our understanding of ${topics.first}?',
            );
          }

          if (keyFindings['location'] != null) {
            questions.add(
              'Why was ${keyFindings['location']} significant for this discovery?',
            );
          }

          if (entities.isNotEmpty) {
            questions.add(
              'What methods did researchers use to analyze ${entities.first}?',
            );
          }
        } else if (entities.isNotEmpty && topics.isNotEmpty) {
          questions.add(
            'What is the broader significance of ${entities.first} in ${topics.first} research?',
          );
          questions.add(
            'How does this discovery compare to previous findings in ${topics.first}?',
          );
        }
        break;
      ...
      case 'explanation':
        if (topics.isNotEmpty) {
          questions.add(
            'What are the main factors that influence ${topics.first}?',
          );
          questions.add(
            'What evidence supports the current understanding of ${topics.first}?',
          );

          if (entities.isNotEmpty) {
            questions.add(
              'How does ${entities.first} contribute to ${topics.first}?',
            );
          }
        }
        ...
        if (contentType == 'technical') {
          questions.add(
            'What are the key technical details that explain this?',
          );
        }
        break;
      ...
      case 'comparison':
        if (entities.length >= 2) {
          questions.add(
            'What are the fundamental differences between ${entities[0]} and ${entities[1]}?',
          );
          questions.add(
            'Which aspects of ${entities[0]} are considered superior?',
          );
          questions.add(
            'What are the trade-offs when choosing between ${entities[0]} and ${entities[1]}?',
          );
        } else if (entities.isNotEmpty && topics.isNotEmpty) {
          questions.add(
            'How does ${entities.first} compare to alternatives in ${topics.first}?',
          );
        }
        break;
      ...
      case 'current_event':
        if (entities.isNotEmpty) {
          questions.add(
            'What recent developments have occurred with ${entities.first}?',
          );
          questions.add(
            'How have experts reacted to the ${entities.first} situation?',
          );
          questions.add(
            'What are the potential implications of this ${entities.first} news?',
          );
        }
      ...
      case 'person':
        if (entities.isNotEmpty) {
          ...
          if (queryLower.contains(
            RegExp(r'\b(convicted|sentenced|trial|charges?)\b'),
          )) {
            ...
            RegExp(r'\b(minister|president|leader|director)\b'),
          )) {
            ...
            questions.add(
              'What are $person\'s most significant contributions?',
            );
      ...
      case 'definition':
        if (topics.isNotEmpty) {
          questions.add(
            'What are the practical applications of ${topics.first}?',
          );
          questions.add(
            'How is ${topics.first} different from related concepts?',
          );
          questions.add(
            'What are common misconceptions about ${topics.first}?',
          );
        }
        break;
      ...
      case 'procedural':
        if (topics.isNotEmpty) {
          questions.add(
            'What are the common challenges when implementing ${topics.first}?',
          );
          questions.add(
            'What tools or resources are recommended for ${topics.first}?',
          );
          questions.add(
            'What best practices should be followed for ${topics.first}?',
          );
        }
        break;
      ...
      case 'analytical':
        if (entities.isNotEmpty && topics.isNotEmpty) {
          questions.add(
            'What are the long-term implications of ${entities.first} for ${topics.first}?',
          );
          questions.add(
            'How might ${entities.first} evolve in the context of ${topics.first}?',
          );
          questions.add(
            'What are different perspectives on the ${entities.first} impact?',
          );
        }
        break;
      ...
      default:
        ...
        if (topics.isNotEmpty) {
          questions.add(
            'What recent developments have occurred in ${topics.first}?',
          );
        }
    }
  ...
  List<String> _generateContentTypeQuestions(
    ...
    switch (contentType) {
      case 'scientific':
        ...
        if (topics.isNotEmpty) {
          questions.add(
            'What are the limitations of this ${topics.first} study?',
          );
        }
        break;
      ...
      case 'technical':
        questions.add(
          'Can you explain the technical implementation in simpler terms?',
        );
        if (topics.isNotEmpty) {
          questions.add(
            'What are real-world applications of this ${topics.first} technology?',
          );
        }
        break;
      ...
      case 'historical':
        if (entities.isNotEmpty) {
          questions.add(
            'How does ${entities.first} fit into the broader historical context?',
          );
        }
        break;
      ...
      case 'business':
        if (entities.isNotEmpty) {
          questions.add(
            'What is ${entities.first}\'s market position and competitive advantage?',
          );
        }
        break;
    }
  ...
  List<String> _generateTemporalQuestions(
    List<String> entities,
    List<String> topics,
  ) {
    final questions = <String>[];
    ...
    if (entities.isNotEmpty) {
      ...
      questions.add(
        'How is the ${entities.first} situation expected to develop?',
      );
    } else if (topics.isNotEmpty) {
      questions.add(
        'What recent advancements have been made in ${topics.first}?',
      );
    } else {
  ...
  String _analyzeContentType(List<ContextChunk> chunks) {
    ...
    for (final chunk in chunks.take(3)) {
      ...
      // Scientific content
      if (RegExp(
        r'\b(study|research|findings?|evidence|data|analysis|hypothesis|'
        r'experiment|peer-reviewed|published|journal)\b',
      ).hasMatch(content)) {
      ...
      // Technical content
      if (RegExp(
        r'\b(algorithm|code|software|hardware|api|framework|'
        r'implementation|architecture|system|protocol)\b',
      ).hasMatch(content)) {
      ...
      // News content
      if (RegExp(
        r'\b(reported|announced|according to|sources? say|yesterday|'
        r'today|breaking|update)\b',
      ).hasMatch(content)) {
      ...
      // Historical content
      if (RegExp(
        r'\b(history|historical|century|era|period|ancient|'
        r'civilization|dynasty)\b',
      ).hasMatch(content)) {
      ...
      // Business content
      if (RegExp(
        r'\b(company|business|market|revenue|profit|stock|'
        r'investment|ceo|startup)\b',
      ).hasMatch(content)) {
      ...
      // Legal content
      if (RegExp(
        r'\b(court|judge|trial|verdict|law|legal|attorney|'
        r'lawsuit|conviction)\b',
      ).hasMatch(content)) {
  ...
  String _analyzeQueryIntent(String queryLower) {
    ...
    scores['discovery'] = _scoreIntent(queryLower, [
      ...
      {
        'pattern': r'\b(study|research|findings?|evidence|data|analysis)',
        'weight': 2.5,
      },
      // Breakthrough terms
      {
        'pattern': r'\b(breakthrough|groundbreaking|significant|important)',
        'weight': 2.0,
      },
      // Change/impact terms
      {
        'pattern': r'\b(rewrite|change|transform|challenge|question)',
        'weight': 2.0,
      },
      // Scientific objects
      {
        'pattern': r'\b(skull|fossil|artifact|specimen|sample|dna)',
        'weight': 1.5,
      },
      // Time/age indicators
    ...
    scores['explanation'] = _scoreIntent(queryLower, [
      ...
      {
        'pattern': r'\b(work|function|operate|mechanism|process)',
        'weight': 2.5,
      },
      // Causation terms
      {
        'pattern': r'\b(because|reason|cause|result|lead|affect)',
        'weight': 2.0,
      },
      // Explanation verbs
    ...
    scores['comparison'] = _scoreIntent(queryLower, [
      ...
      // Evaluation terms
      {
        'pattern': r'\b(better|worse|superior|inferior|advantage)',
        'weight': 2.5,
      },
    ...
    scores['current_event'] = _scoreIntent(queryLower, [
      ...
      // News terms
      {
        'pattern': r'\b(news|report|announcement|update|breaking)',
        'weight': 3.0,
      },
      // Event terms
    ...
    scores['person'] = _scoreIntent(queryLower, [
      ...
      // Achievement terms
      {
        'pattern': r'\b(achievement|accomplish|award|recognition)',
        'weight': 2.5,
      },
      // Position/role terms
      {
        'pattern': r'\b(minister|president|ceo|director|leader|founder)',
        'weight': 2.5,
      },
      // Legal terms (person-related)
      {
        'pattern': r'\b(convicted|sentenced|accused|charged|trial)',
        'weight': 2.5,
      },
      // Personal terms
    ...
    scores['procedural'] = _scoreIntent(queryLower, [
      ...
      {
        'pattern': r'\b(how to|steps|guide|tutorial|instructions)',
        'weight': 4.0,
      },
      // Action verbs
      {
        'pattern': r'\b(install|setup|configure|create|build|make)',
        'weight': 2.5,
      },
      // Process terms
    ...
    scores['analytical'] = _scoreIntent(queryLower, [
      // Analysis terms
      {
        'pattern': r'\b(analyz|impact|effect|consequence|implication)',
        'weight': 3.0,
      },
      // Depth indicators
    ...
    print(
      '   🎯 Intent scores: ${scores.entries.map((e) => "${e.key}:${e.value.toStringAsFixed(1)}").join(", ")}',
    );
    print(
      '   ✨ Detected intent: $detectedIntent (score: ${maxScore.toStringAsFixed(1)})',
    );
  ...
  Map<String, String> _extractKeyFindingsFromContent(
    List<ContextChunk> chunks,
  ) {
    ...
    for (final chunk in chunks.take(2)) {
      ...
      final discoveryMatch = RegExp(
        r'(skull|fossil|remains?|artifact|discovery|finding)',
      ).firstMatch(content);
      ...
      final locationMatch = RegExp(
        r'(from|in|at)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)',
      ).firstMatch(chunk.content);
      ...
      final timeMatch = RegExp(
        r'(\d+(?:,\d+)?)\s*(?:million|thousand)?\s*year',
      ).firstMatch(content);
  ...
  bool _isWellFormedQuestion(String question) {
    ...
    final startsCorrectly = RegExp(
      r'^(What|How|Why|When|Where|Who|Which|Can|Could|Should|Would|Is|Are|Do|Does|Did)',
    ).hasMatch(question);
  ...
  List<String> _generateSmartFallbacks(
    List<String> entities,
    List<String> topics,
  ) {
    ...
    if (entities.isNotEmpty) {
      fallbacks.add(
        'What are the broader implications of this ${entities.first}?',
      );
    ...
    for (final chunk in chunks.take(3)) {
      ...
      for (int i = 0; i < titleWords.length - 1; i++) {
        ...
        if (word1.length > 3 &&
            ...
            !_isCommonWord(word1) &&
            !_isCommonWord(word2) &&
            !_isNumericOrDate(word1) &&
            !_isNumericOrDate(word2)) {
      ...
      for (int i = 0; i < words.length - 1; i++) {
        ...
        if (word1[0].toUpperCase() == word1[0] &&
          ...
          if (chunk.citations.isNotEmpty) {
            final title = chunk.citations.first.document.title;
            if (!_isTitleNoisy(title) &&
                title.toLowerCase().contains(entity.toLowerCase())) {
              score += 3.0;
            }
          }
      ...
      for (final word in words) {
        ...
        if (cleaned.length > 3 &&
          ...
          if (chunk.citations.isNotEmpty) {
            final title = chunk.citations.first.document.title;
            if (!_isTitleNoisy(title) &&
                title.toLowerCase().contains(cleaned.toLowerCase())) {
              score += 2.0;
            }
          }
  ...
  bool _isKnownEntityPattern(String entity) {
    ...
    // Geographic patterns
    if (RegExp(
      r'\b(china|india|america|europe|africa|asia|russia|japan|'
      r'korea|australia|mexico|brazil|canada|france|germany)\b',
    ).hasMatch(entityLower)) {
      return true;
    }

    // Scientific/archaeological patterns
    if (RegExp(
      r'\b(yunxian|dali|peking|olduvai|homo|sapiens|erectus|'
      r'neanderthal|denisovan)\b',
    ).hasMatch(entityLower)) {
      return true;
    }

    // Institutional patterns
    if (entity.toLowerCase().contains(
      RegExp(
        r'\b(university|institute|museum|'
    ...
    if (RegExp(
      r'\b(dr|prof|president|minister|director)\b',
    ).hasMatch(entityLower)) {
      return true;
    }
  ...
  String resolveContextualQuery(
    ...
    final recentMessages = conversationHistory.length > maxHistoryMessages
        ? conversationHistory
              .skip(conversationHistory.length - maxHistoryMessages)
              .toList()
        : conversationHistory;
    ...
    for (final message in recentMessages) {
      ...
      final entities =
          RegExp(r'\b[A-Z][A-Z]+\b|\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b')
    ...
    final hasReference = RegExp(
      r'\b(previous|earlier|before|last|above|you said|you mentioned)\b',
    ).hasMatch(queryLower);
    ...
    final isContinuation = queryLower.startsWith(
      RegExp(r'^(and|also|but|however|plus|moreover)\b'),
    );
    ...
    if (hasPronoun && conversationEntities.isNotEmpty) {
      ...
      if (RegExp(
        r'^(who|what|when|where|which|how)\b',
        caseSensitive: false,
      ).hasMatch(query)) {
      ...
      resolvedQuery = query.replaceFirst(
        RegExp(
          r'^(and|also|but|however|plus|moreover)\s+',
          caseSensitive: false,
        ),
        '',
      );
      ...
      if (mainEntity.isNotEmpty &&
          !resolvedQuery.toLowerCase().contains(mainEntity.toLowerCase())) {
      ...
      if (mainEntity.isNotEmpty &&
          !query.toLowerCase().contains(mainEntity.toLowerCase())) {
