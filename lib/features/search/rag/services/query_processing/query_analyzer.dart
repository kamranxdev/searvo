...
class QueryAnalyzer {
  ...
  QueryAnalysis analyzeQuery(String query) {
    ...
    final subQueries = complexity['level'] == 'complex' || complexity['level'] == 'moderate'
    ...
    print('   Context dependency: ${contextDependency['level']} (${contextDependency['confidence']})');
  ...
  Map<String, dynamic> analyzeContextDependency(String query) {
    ...
    final referencePatterns = [
      RegExp(r'\b(previous|earlier|before|last|above|mentioned|said|you said|you mentioned)\b'),
    ...
    final attributeQuestions = [
      RegExp(r'^(who|what|when|where|which) (is|are|was|were) (the|its|their|his|her)\b'),
    ...
    final entities = _extractEntities(query); // Need entities to check specificity
    final hasProperNouns = RegExp(r'\b[A-Z][a-z]+\b').hasMatch(query);
    final hasQuestionWord = RegExp(r'^(who|what|when|where|why|how|which|explain|describe|tell me about)\b')
        .hasMatch(lowerQuery);
    final isLongQuery = words.length > 6;
    final hasSpecificTopic = entities.isNotEmpty || hasProperNouns;
    
    // Calculate dependency score
    double dependencyScore = indicators.values.fold(0.0, (sum, val) => sum + val);
    
    // Reduce score for independent indicators
    if (hasSpecificTopic && isLongQuery) dependencyScore -= 0.3;
    if (hasQuestionWord && !indicators.containsKey('pronouns')) dependencyScore -= 0.1;
  ...
  Map<String, dynamic> _detectIntent(String query) {
    ...
    final transactionalKeywords = ['buy', 'purchase', 'download', 'price', 'order', 'shop', 'deal'];
    if (transactionalKeywords.any((kw) => lowerQuery.contains(kw))) {
      return {'type': 'transactional', 'confidence': 0.85};
    }
    
    // Research/academic intent
    final researchKeywords = ['research', 'study', 'paper', 'journal', 'scholar', 'academic', 'thesis'];
    if (researchKeywords.any((kw) => lowerQuery.contains(kw))) {
      return {'type': 'research', 'confidence': 0.9};
    }
    
    // News intent
    final newsKeywords = ['news', 'latest', 'recent', 'today', 'breaking', 'update'];
    if (newsKeywords.any((kw) => lowerQuery.contains(kw))) {
      return {'type': 'news', 'confidence': 0.85};
    }
    
    // How-to/instructional intent
    if (RegExp(r'\b(how to|how do|tutorial|guide|step|instructions?)\b').hasMatch(lowerQuery)) {
      return {'type': 'instructional', 'confidence': 0.9};
    }
    
    // Comparison intent
    if (RegExp(r'\b(vs|versus|compare|comparison|better|difference|between)\b').hasMatch(lowerQuery)) {
  ...
  Map<String, dynamic> _assessComplexity(String query) {
    ...
    final hasMultipleClauses = query.contains(',') || 
                                query.contains('and') || 
                                query.contains('or') ||
                                query.contains(';');
    final hasQuestions = query.contains('?');
    final questionWords = ['what', 'why', 'how', 'when', 'where', 'who', 'which', 'explain', 'describe'];
    final hasQuestionWord = questionWords.any((qw) => query.toLowerCase().contains(qw));
    
    int complexityScore = 0;
    
    // Word count factor
    if (wordCount > 15) complexityScore += 3;
    else if (wordCount > 10) complexityScore += 2;
  ...
  Map<String, dynamic> _extractTemporalContext(String query) {
    ...
    if (RegExp(r'\b(this month|past month|last month)\b').hasMatch(lowerQuery)) {
      return {'hasTemporalContext': true, 'recency': 'month', 'type': 'recent'};
    }
    
    if (RegExp(r'\b(this year|past year|last year|\d{4})\b').hasMatch(lowerQuery)) {
  ...
  List<String> _extractEntities(String query) {
    ...
    final capitalizedMatches = RegExp(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b').allMatches(query);
    for (final match in capitalizedMatches) {
      final entity = match.group(0)!;
      // Skip common question words
      if (!['What', 'Why', 'How', 'When', 'Where', 'Who', 'Which'].contains(entity)) {
        entities.add(entity);
      }
    }
  ...
  List<String> _generateSubQueries(
    ...
    // For comparison queries, split into individual queries
    if (intent['type'] == 'comparison') {
      final vsMatch = RegExp(r'(.+?)\s+(vs|versus|compared to)\s+(.+)', caseSensitive: false)
          .firstMatch(query);
      if (vsMatch != null) {
        ...
        subQueries.add('$item1 features benefits');
        subQueries.add('$item2 features benefits');
      }
    }
    
    // For multi-part questions with "and"
    if (lowerQuery.contains(' and ') && lowerQuery.contains('?')) {
      final parts = query.split(RegExp(r'\s+and\s+', caseSensitive: false));
      if (parts.length > 1) {
        subQueries.addAll(parts.map((p) => p.trim()).where((p) => p.isNotEmpty));
      }
    }
    
    // For "how" questions, add related context queries
    if (lowerQuery.startsWith('how')) {
      final topic = query.replaceFirst(RegExp(r'^how\s+(to\s+)?', caseSensitive: false), '').trim();
      if (topic.isNotEmpty) {
        subQueries.add('$topic tutorial');
        subQueries.add('$topic guide');
        subQueries.add('$topic best practices');
      }
    }
    
    // For "why" questions, add related "what" and "how" queries
    if (lowerQuery.startsWith('why')) {
      final topic = query.replaceFirst(RegExp(r'^why\s+', caseSensitive: false), '').trim();
      if (topic.isNotEmpty) {
        subQueries.add('what causes $topic');
        subQueries.add('$topic explanation');
      }
    }
    ...
    return subQueries.take(4).toList(); // Limit to 4 sub-queries
  }

  /// Determine which search types to use
  List<String> _determineSearchTypes(String query, Map<String, dynamic> intent) {
    final types = <String>['general']; // Always include general search
    final lowerQuery = query.toLowerCase();
    
    // News search
    if (intent['type'] == 'news' || 
        RegExp(r'\b(news|latest|recent|today|breaking)\b').hasMatch(lowerQuery)) {
      types.add('news');
    }
    
    // Scholar/academic search
    if (intent['type'] == 'research' ||
        RegExp(r'\b(research|study|paper|scholar|academic|journal)\b').hasMatch(lowerQuery)) {
      types.add('scholar');
    }
    
    // Shopping search
    if (intent['type'] == 'transactional' ||
        RegExp(r'\b(buy|price|shop|purchase|deal|discount)\b').hasMatch(lowerQuery)) {
      types.add('shopping');
    }
    
    // Image search
    if (RegExp(r'\b(image|photo|picture|visual|gallery)\b').hasMatch(lowerQuery)) {
      types.add('images');
    }
    
    // Video search
    if (RegExp(r'\b(video|watch|tutorial|demonstration|review)\b').hasMatch(lowerQuery)) {
      types.add('videos');
    }
