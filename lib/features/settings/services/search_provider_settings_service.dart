...
class SearchProviderSettingsService {
  ...
  Future<bool> setSearXNGEndpoint(String endpoint) async {
    return await _settingsService.setCustomSetting(_searxngEndpointKey, endpoint);
  }

  String getSearXNGEndpoint() => 
    _settingsService.getCustomSetting<String>(_searxngEndpointKey, 'http://localhost:4000') ??
    'http://localhost:4000';

  bool hasSearXNGEndpoint() {
    final endpoint = _settingsService.getCustomSetting<String>(_searxngEndpointKey, null);
  ...
  Future<bool> testSearXNGConnection({String? customEndpoint}) async {
    ...
    final searchUri = Uri.parse('$baseTrimmed/search').replace(queryParameters: {
      'q': 'test',
      'format': 'json',
      'pageno': '1',
    });
    ...
    try {
      final searchResp = await http.get(searchUri, headers: {'Accept': 'application/json'}).timeout(const Duration(seconds: 5));
